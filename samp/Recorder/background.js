...
(function() {
    ...
    async function handleGmeetAutoStart(message, sender) {
        ...
        const hasPermission = result.autoRecordPermissions?.[service] || false;

        if (!hasPermission) {
            console.log("❌ Auto recording denied - no permission for", service);
    ...
    function handleTeamsTabUpdate(tabId, tab) {
        ...
        chrome.storage.local.get(['autoRecordPermissions'], (result) => {
            const hasPermission = result.autoRecordPermissions?.['teams'] || false;
            if (hasPermission) {
...
(function() {
    ...
    async function handleGmeetAutoStart(message, sender) {
        ...
        console.log("⏰ Will start recording in 2 seconds...");
        setTimeout(() => {
            console.log("🚀 EXECUTING: startRecordingForTab for GMeet");
            startRecordingForTab(sender.tab.id, 'gmeet');
        }, 2000);
    ...
    function handleTeamsTabUpdate(tabId, tab) {
        ...
        if (currentRecordingTab === tabId) {
            console.log("⚠️ Already recording this Teams tab, skipping auto-start");
            return;
        }
    ...
    function handleTeamsAutoStart(sender) {
    const timestamp = new Date().toLocaleTimeString();
    console.log(`🎬 TEAMS Auto starting recording at ${timestamp}`);
    console.log("📍 Source tab:", sender.tab.id, sender.tab.url);
    
    console.log("🚀 EXECUTING: startRecordingForTab for Teams");
    startRecordingForTab(sender.tab.id, 'teams');
    return { success: true };
}
    ...
    function startRecordingForTab(tabId, service) {
        if (currentRecordingTab === tabId) {
            console.log("⚠️ Already recording this tab:", tabId);
            return;
        }

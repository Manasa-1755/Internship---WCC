...
document.addEventListener("DOMContentLoaded", async () => {
    console.log("🔍 Recorder popup opened");
...
async function initializePopup() {
    ...
    // Always show recording UI since user manually selects service
    showRecordingUI();
...
function selectService(service) {
    ...
    // Load service-specific auto record permission
...
function showRecordingUI() {
    ...
    // Don't update service indicator text since user selects manually
...
function updateUIForRecording(recordingTime) {
    ...
    if (warningElement) warningElement.style.display = currentService === 'teams' ? "block" : "none";
...
function updateUIForReady() {
    ...
    if (statusElement) {
        if (activeTabId && currentService) {
            ...
            statusElement.textContent = "✅ Extension is ready";
        }
    }
    
    if (startBtn) startBtn.textContent = "Start Recording";
    if (warningElement) warningElement.style.display = (activeTabId && currentService === 'teams') ? "block" : "none";
...
async function checkAutoRecordPermission() {
    ...
    // Get service-specific auto record permission
...
console.log("🔄 Checking if currently in meeting...");
chrome.tabs.sendMessage(activeTabId, { 
    action: "checkMeetingStatus" 
}, (response) => {
    if (chrome.runtime.lastError) {
        console.log("❌ Cannot check meeting status:", chrome.runtime.lastError.message);
        return;
    }
    ...
    if (response && response.isInMeeting && !response.recording) {
        console.log("🎯 Auto-record enabled mid-meeting - starting recording NOW");
        
        // SEND DIRECTLY TO CONTENT SCRIPT INSTEAD OF BACKGROUND
        chrome.tabs.sendMessage(activeTabId, { 
            action: "startAutoRecordingMidMeeting" 
        }, (startResponse) => {
            console.log("🚀 Auto-start response:", startResponse);
        });
    } else {
        console.log("ℹ️ Not starting recording - conditions not met:", {
            hasResponse: !!response,
            isInMeeting: response?.isInMeeting,
